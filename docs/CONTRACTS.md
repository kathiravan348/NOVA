...
> Pydantic mirrors: `backend/libs/nova_contracts` (so far: User, ApiError, LoginRequest, AuditEntry, Page, BrokerAccount(+Create), BrokerProfile, KiteApp(+Update, KiteKeysUpdate, KitePassphrase), RateLimit, RateLimitUpdate, RecorderSettings(+Update), DataJob(+Create, ArchiveJobCreate, DataJobPlan, DataJobPlanRequest, DownloadSettings(+Update)), Instrument, Candle, UniverseEntry(+Write), MarketIndex, RealtimeMessage, Charges, Strategy (+ spec tree, write bodies), StrategyStats, BacktestRun(+Create), BacktestResult, Trade), checked with `nova_testing.parity`.
...
| DataJobPlan / DataJobPlanRequest | Plan of a download shown before Start; `DataJob.plan` of a `draft` (D57). Endpoints in NOVA-093 | — | Relay |
| DownloadSettings | Pace during market hours (`slow`/`full`, D57). Endpoints in NOVA-093 | — | Relay |
