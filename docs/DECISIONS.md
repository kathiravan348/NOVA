...
| D30 | 2026-09-23 | Checks: library packages have no build step (used from `src`); `tsc` runs only in `typecheck`; ESLint, Prettier and `tsc` cache under `node_modules/.cache` (tsbuildinfo git-ignored); Vitest uses the `threads` pool; `review:check` runs lint, typecheck, test and format in parallel, then build. While working, agents run only the checks for the packages they touch | The full check took about 6 min on Windows, mostly repeated or uncached work |
