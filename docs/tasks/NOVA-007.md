...
**Rulebook issues found:** `Button.tsx` primary uses `text-[white]`, an arbitrary colour outside the tokens (D15 intent). Contrast passes in both themes, but there is no `on-action` token. Accepted for now; tracked in NOVA-025.
**Follow-up tasks created:** NOVA-025 (add `on-action` text token; swap `text-[white]` in Button).
