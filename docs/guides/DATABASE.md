...
> State as of 26 Sep 2026 (migrations `0001`–`0014`, NOVA-101). Source of truth: `backend/libs/nova_db/src/nova_db/models/`.
...
| **backtest_runs** | Each test run **and the job queue** for the backtest worker (it claims the oldest `queued` row with `FOR UPDATE SKIP LOCKED`). | `id`, `strategy_id` + `strategy_version` → strategy_versions, `name`, `universe` (JSON: chosen symbols or an index), `status` (`queued`/`running`/`completed`/`failed`), `date_from`, `date_to`, `initial_capital_paise`, `benchmark` (`NIFTY 50` or null), `created_at`, `started_at`, `finished_at`, `error` (only when failed); progress (migration 0014, D58): `stage` (null until started, then `loading`/`signals`/`simulating`/`saving`/`done`), `progress_percent` (0–100), `symbols_done`, `symbols_total`, `bars_done`, `bars_total`, `trades_so_far` (all ≥ 0, default 0), `simulated_to` (IST date reached). The worker writes them in their own transaction while the run's own transaction is open |
...
| **candles** | Price bars (OHLCV). **TimescaleDB hypertable** on `ts`, 30-day chunks, compressed after 30 days (segments: `exchange`, `symbol`, `timeframe`; order `ts`; policy every 6 h, migration 0013); writes into old chunks still work and are recompressed by the policy. Daily bars are stored at 00:00 IST. DB check: high ≥ open/close ≥ low > 0. | PK (`exchange`, `symbol`, `timeframe`, `ts`), `open_paise`, `high_paise`, `low_paise`, `close_paise`, `volume`; `timeframe` ∈ `1m 3m 5m 15m 30m 1h 1d` |
